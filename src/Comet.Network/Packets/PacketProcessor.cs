...
namespace Comet.Network.Packets
{
        ...
        where TClient : TcpServerActor
    {
        ...
        public void Queue(int partition, byte[] packet)
        {
            if (!CancelWrites.IsCancellationRequested)
                Channels[partition].Writer.TryWrite(new Message
                {
                    Actor = null,
                    Packet = packet
                });
        ...
        protected async Task DequeueAsync(Channel<Message> channel)
        {
            while (!CancelReads.IsCancellationRequested)
            {
                ...
                if (msg != null) await Process(msg.Actor, msg.Packet).ConfigureAwait(false);
            }
        }
        ...
        public new async Task StopAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (Channel<Message> channel in Channels)
